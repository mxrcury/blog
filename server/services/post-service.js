...
class PostService {
  async getAll(userId) {
    ...
    const posts = allPosts.rows.map((post) => {
      ...
      delete likedPost.created_at;
      delete likedPost.created_by;
      return likedPost;
    });
    return posts;
  }
  async createPost(post, userId) {
    ...
    const { rows: [postFromDb] } = await pool.query(
    ...
    const createdPost = {
      ...postFromDb,
      isLiked: postFromDb.likes.includes(userId),
      likesQty: postFromDb.likes.length,
      createdAt: postFromDb.created_at,
      createdBy: postFromDb.created_by,
    };
    delete createdPost.likes;
    delete createdPost.created_by;
  ...
  async toggleLikeOnPost(id, userId) {
    ...
    const likedPost = await pool.query(`UPDATE posts SET likes = array_append(likes,$1) WHERE id = $2 RETURNING*;`, [
      userId,
      id,
    ]);
  ...
  async getOnePost(postId, userId) {
    ...
    const postComments = await pool.query(`SELECT * FROM post_comments WHERE post_id = $1 ORDER BY id DESC`, [
      postData.rows[0].id,
    ]);
    const post = {
      ...postData.rows[0],
      createdAt: postData.rows[0].created_at,
      createdBy: postData.rows[0].created_by,
