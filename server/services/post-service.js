...
class PostService {
    async getAll(userId){
        ...
        const posts = allPosts.rows.map(post=>{
            ...
            if(isLikedByUser){
                const likedPost = {...post,isLiked:true,likesQty:post.likes.length}
    ...
    async createPost(post){
        ...
        const createdPost = await pool.query(`INSERT INTO posts (title,content, created_by,created_at, likes,caption) values ($1,$2,$3,$4, $5,$6) RETURNING*;`,[title,content,created_by,created_at, likes, caption])
    ...
    async toggleLikeOnPost(id,userId){
        // TODO:Make toggling like on post
        const post = await pool.query(`SELECT * FROM posts WHERE id = $1;`,[id])
        if(post.rows[0] && post.rows[0].likes.includes(userId)){
            const unlikedPost = post.rows[0].likes.filter(id=>id!==userId)
            const likedPost = await pool.query(`UPDATE posts SET likes = $1 WHERE id = $2 RETURNING*;`,[unlikedPost, id])
            return likedPost.rows[0]
        }
        const likedPost = await pool.query(`UPDATE posts SET likes = array_append(likes,$1) WHERE id = $2 RETURNING*;`,[userId, id])
        return likedPost.rows
    }
    async getOnePost(postId,userId) {
        const postData = await pool.query(`SELECT * FROM posts WHERE id = $1`,[postId])
        const postCreator = await pool.query(`SELECT id FROM users WHERE username = $1`,[postData.rows[0].created_by])
        const postComments = await pool.query(`SELECT * FROM post_comments WHERE post_id = $1 ORDER BY id DESC`,[postData.rows[0].id])
        const post = {
          ...postData.rows[0],
    ...
    async addComment(post) {
        ...
        const addedComment = await pool.query(`INSERT INTO post_comments (post_id,created_by,created_at, text) values ($1,$2,$3,$4) RETURNING*;`,[postId,createdBy,createdAt,text])
